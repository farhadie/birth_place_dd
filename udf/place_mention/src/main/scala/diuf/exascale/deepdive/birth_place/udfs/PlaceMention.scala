...
object PlaceMention extends Deepdive{
  def main(args: Array[String]): Unit = {
    ...
    val mentions = inputDF.flatMap{
      r => {
		...
		val tokens = r(2).asInstanceOf[mutable.WrappedArray[String]]
        ...
        ner_tags.zipWithIndex.foreach{
          case (tag:String, i:Int) => {
            if(tag == "LOCATION" && (i == 0 || (ner_tags(i-1) != "LOCATION" && tokens(i-1)!=",") || (i>1 && tokens(i-1)=="," && ner_tags(i-2) != "LOCATION"))){
        ...
        first_indexes.foreach{
          case(first_index:Int) => {
            ...
            breakable{
				while(true){
					if (end_index+1 < num_tokens && ner_tags(end_index+1) == "LOCATION" && tokens(end_index) == ",") end_index += 2
					else if (end_index < num_tokens && ner_tags(end_index) == "LOCATION") end_index += 1
					else break
				}
			}
            end_index -= 1
